...
export default function PackageDetailScreen({navigation}: Props) {
  ...
  const { markAsFavourite, loadingFavourite, getPackageDetail, packageDetail } =
    usePackages();
      ...
      <ScrollView style={{ backgroundColor: activeColors.primary }}>
        ...
        <View style={styles.infoContainer}>
          <View style={styles.infoHeader}>
            ...
            <TouchableOpacity
              onPress={() =>
                markAsFavourite(packageDetail.id, !packageDetail.is_favourite)
              }
            >
              <MaterialIcons
                name={
                  packageDetail?.is_favourite ? "favorite" : "favorite-border"
                }
                size={30}
                color={
                  packageDetail?.is_favourite
                    ? Colors.brand
                    : activeColors.light
                }
              />
          ...
          {paragraphs?.map((paragraph: any, index: any) => (
            <Text
              key={index}
              style={{ fontFamily: "outfit", color: activeColors.light }}
            >
              {paragraph}
            </Text>
          ))}
